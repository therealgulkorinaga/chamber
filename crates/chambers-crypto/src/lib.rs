//! Cryptographic primitives for Chambers.
//!
...
//!
//! Phase 2: memory hardening — mlock, MADV_DONTDUMP, guard buffer.
...
impl CryptoProvider {
    ...
    {
        let keys = self.world_keys.lock().unwrap();
        let key = keys.get(&world_id).ok_or(CryptoError::KeyNotFound(world_id))?;
        Ok(f(key))
    }
}
